    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <a className="btn ghost" style={{ fontSize: 11 }} href="/api/admin/backend/suppliers/export" download="proveedores.xlsx">Exportar Excel</a>
          ...
          <button className="btn" style={{ fontSize: 11 }} onClick={openCreate}>+ Nuevo Proveedor</button>
        </div>
      </div>
      ...
      <div style={{ background: 'white', border: '1px solid var(--line)' }}>
        ...
        {!loading && suppliers.map((s, i) => (
          <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 100px 140px', padding: '14px 24px', borderBottom: i < suppliers.length - 1 ? '1px solid var(--line)' : 'none', alignItems: 'center', opacity: s.active ? 1 : 0.5 }}>
            <div>
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: 14, fontWeight: 500 }}>{s.companyOrFullName}</div>
              <div className="mono" style={{ fontSize: 9, color: 'var(--ink-mute)' }}>{IVA_CONDITION_LABEL[s.ivaCondition]}</div>
            </div>
            <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{s.taxId || '-'}</div>
            <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{s.phone || s.cell || s.email || '-'}</div>
            <div>
              <span className="mono" style={{ fontSize: 9, color: s.active ? '#065F46' : '#991B1B' }}>{s.active ? 'Activo' : 'Inactivo'}</span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => openEdit(s)} style={linkBtn}>Editar</button>
              {s.active ? (
                <button onClick={() => onDelete(s)} style={{ ...linkBtn, color: '#991B1B' }}>Eliminar</button>
              ) : (
                <button onClick={() => onActivate(s)} style={{ ...linkBtn, color: '#065F46' }}>Reactivar</button>
              )}
            </div>
          </div>
        ))}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'grid', placeItems: 'center', zIndex: 100, padding: 20 }}>
      <div style={{ background: 'white', width: 680, maxHeight: '90vh', overflowY: 'auto', padding: 32 }}>
        <h2 style={{ fontSize: 20, fontWeight: 500, margin: '0 0 20px' }}>{isNew ? 'Nuevo Proveedor' : 'Editar Proveedor'}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Field label="Categoría Compras"><input value={form.purchasesCategory} onChange={(e) => set('purchasesCategory', e.target.value)} style={inputStyle} /></Field>
          <Field label="Descuento General %"><input type="number" value={form.purchasesDiscountPercent} onChange={(e) => set('purchasesDiscountPercent', e.target.value)} style={inputStyle} /></Field>
        </div>
        <Field label="Nota Interna">
          <textarea value={form.noteInternal} onChange={(e) => set('noteInternal', e.target.value)} rows={2} style={{ ...inputStyle, width: '100%', resize: 'vertical' }} />
        </Field>
